    ...
    def draw_slam_state(self, ax) -> None:
        ...
        if self.number_landmarks() > 0:
            for i in range(len(self.taglist)):
                ...
                ax.annotate(str(self.taglist[i]),(self.markers[0,i], self.markers[1,i]))
        ...
        ax.set_xlim(-2+self.robot.state[0],2+self.robot.state[0])
        ax.set_ylim(-2+self.robot.state[1],2+self.robot.state[1])
